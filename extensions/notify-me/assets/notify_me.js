class ModalNotification {
    ...
    init() {
        if (this.openBtn) this.openBtn.addEventListener("click", this.openModal.bind(this));
    ...
    handleFormSubmit(e) {
        ...
        const email = this.contactEmail.value.trim().length > 0 ? this.contactEmail.value.trim() : null;
        const number = this.contactNumber.value.trim().length > 0 ? this.contactNumber.value.trim() : null;
        ...
        fetch('http://localhost:3000/notification/subscribe', {
            ...
            .catch(err => {
                console.log(err);
                this.errorMessage.textContent = "An error occurred while processing your request. Please try again.";
            });
    ...
    tracksTheUrlChanges() {
        ...
        const checkUrlChanges = () => {
            ...
            if (JSON.stringify(currentParams) !== JSON.stringify(lastParams)) {
                ...
                if (isNaN(variantId)) {
                    const availableVariant = this.variants.find(variant => variant.available);
                    if (availableVariant) {
                ...
                const foundVariant = this.variants.find(variant => variant.id === variantId);
                if (foundVariant) {
